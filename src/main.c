...
int maparray [HEIGHT][WIDTH];

void recordarray(int x, int y)
{
...
writeString_P("Array:  X   Y \n");

int n,m;

for (n=0;n<10;n++) //HEIGHT
{
		writeString_P("		");
		writeInteger(maparray[n][0], DEC);
		writeString_P("	");
		writeInteger(maparray[n][1], DEC);
		writeString_P("\n");
...
//writeString_P("measured distance R and L ");
...
void mapping(void)
{
	...
	task_RP6System();
		...
		//move(40, FWD, DIST_MM(200), BLOCKING);
	while(bumpnumber<HEIGHT)
		{
			if(bumper_left||bumper_right||(bumper_left&&bumper_right)) 
			{
				
				bumpnumber++;
				
				while(bumper_left||bumper_right||(bumper_left&&bumper_right))
				{
				moveAtSpeed(0,0);
				task_RP6System();
				}
				recordarray(getdistance(), bumpnumber);
			}
			moveAtSpeed(20,20);
			task_RP6System();
	...
	//writeString_P("measured distance R and L ");
	//writeInteger(getdistance(),DEC);
		
	printarray();
		
	//writeString_P("\n");
	//writeInteger(getLeftDistance()/4.20,DEC);
...
void start_distancemeasure(void)
{
mleft_dist=0;
mright_dist=0;
}
...
writeString_P("measured distance: \n");
writeInteger(getdistance(),DEC);
writeString_P("\n");
...
{			
	...
	{		
		case FWD :		
			//do nothing
			writeString_P("//do nothing \n");
				
			break;	
		case BWD :		
			direction=direction+2;	
			writeString_P("direction=direction+2;	 \n");
			break;	
		case LEFT: 		
			direction--;	
			writeString_P("direction--; \n");
			break;	
		case RIGHT :		
			direction++;	
			writeString_P("direction++; \n");
	...
	writeString_P("getDirection() \n");
	writeInteger(getDirection(),DEC);
	writeString_P("direction: \n");
...
switch(compass)			
	{		
		case NORTH:	
		writeString_P("NORTH \n");
		maparray[array_index][1]=read_distance()+maparray[array_index-1][1]; //y postive
		maparray[array_index][0]=maparray[array_index-1][0]; //x not changed
		break;	
			
		case EAST:	
		writeString_P("EAST \n");	
		maparray[array_index][1]=maparray[array_index-1][1]; //y not changed
		maparray[array_index][0]=read_distance()+maparray[array_index-1][0]; //x postive
		
		break;	
			
		case SOUTH:	
		writeString_P("SOUTH \n");	
		maparray[array_index][1]=maparray[array_index-1][1]-read_distance(); //y negative
		maparray[array_index][0]=maparray[array_index-1][0]; //x not changed
		break;	
		...
		writeString_P("WEST \n");
		maparray[array_index][1]=maparray[array_index-1][1]; //y not changed
		maparray[array_index][0]=maparray[array_index-1][0]-read_distance();	//x negative
		break;	
	}		
...
int newmaparray[20][20];


void coordinatestomap(void)
{
				...
				{		
						...
						{												
							...
							for (int i=0; i<20;i++)
							{
								...
								if ((n*200 <= maparray[i][0]) && (maparray[i][0] < (1+n)*200) && ( m*200 <= maparray[i][1]) && ( maparray[i][1] < (1+m)*200 ) )
								{
									newmaparray[n][m]=1;
									//doesnt register below x<25 and y<25
								}	
...
int n, m;
		...
		for (m=0;m<20;m++)
		{
					for (n=0;n<20;n++)
					{
							writeInteger(newmaparray[n][m], DEC);
							writeString_P("\t");
...
writeString_P("initarray completed\n");
...
void runcommand(int command)
{
	...
	{	
		case Move_Straight:
			writeString_P("Move_Straight \n");
			setMotorDir(FWD,FWD);
			start_distancemeasure();
			changeDirection(FWD);
			moveAtSpeed(100,102);			
			task_RP6System();
			updatedirection();
			...
			writeString_P("moveAtSpeed(60,60) \n");
					
			while((bumper_right||bumper_left)!=1)
			{
		...
		case TurnRight:
			writeString_P("TurnRight \n");
			//start_distancemeasure();
			
			rotate(40, RIGHT, 107, BLOCKING);
		...
		case CertainMove:
			writeString_P("CertainMove \n");
			start_distancemeasure();
			changeDirection(FWD);
			move(60, FWD, DIST_MM(200), false);
			updatedirection();
			readcompass();
			
			writeString_P("MovementNotComplete \n");
			while(!isMovementComplete())
			{
				
				if((bumper_right||bumper_left)!=0) 
				{
					writeString_P("Hit a BUM ;) \n");	
					moveAtSpeed(0,0);
					read_distance();
					record_inarray();
					
					//start_distancemeasure();
					move(60, BWD, DIST_MM(50), true);
					updatedirection();
					readcompass();
			...
			writeString_P("MovementComplete \n");
			changeDirection(FWD);		
		...
		case TurnLeft:
			writeString_P("TurnLeft \n");
			//start_distancemeasure();
			
			rotate(40, LEFT, 107, BLOCKING);
			updatedirection();
			readcompass();
			changeDirection(FWD);
			//read_distance();
...
/*
void pointtopoint(void)
{

//point 5
int distance, xdif, ydif;
ydif=maparray[5][1]-maparray[0][1];
xdif=maparray[5][0]-maparray[0][0];

distance= (xdif`*xdif + ydif*ydif);
distance= sqrt(distance);

omega= intan(ydif/xdif);
}
*/
...
//	createNewDisplay(8, "CURRENT RIGHT", DISPLAY_ANALOG, "0", "1023");
	...
	while(true)
	{
		...
		writeString_P("# 3 - Voltage Sensor Test         # m - mapping w distance          #\n"); 
		writeString_P("# 4 - Bumper Test                 # n - navigational map            #\n"); 
