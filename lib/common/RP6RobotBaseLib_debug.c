...
/* ****************************************************************************
 * ****************************************************************************
 * File: RP6RobotBaseLib.c  ********DEBUG VERSION********
 * Version: 1.0
...
volatile uint16_t ENCL_buffer_values[60];
volatile uint16_t ENCL_buffer_time[60];
volatile uint8_t  ENCL_trig = TRIG_WAIT;
volatile uint16_t ENCL_k = 0;
volatile uint16_t ENCL_time = 0;

volatile uint16_t ENCL_min_l_time = 65535;
volatile uint16_t ENCL_min_h_time = 65535;
volatile uint16_t ENCL_max_l_time = 0;
volatile uint16_t ENCL_max_h_time = 0;


volatile uint16_t ENCR_buffer_values[60];
volatile uint16_t ENCR_buffer_time[60];
volatile uint8_t  ENCR_trig = TRIG_WAIT;
volatile uint16_t ENCR_k = 0;
volatile uint16_t ENCR_time = 0;

volatile uint16_t ENCR_min_l_time = 65535;
volatile uint16_t ENCR_min_h_time = 65535;
volatile uint16_t ENCR_max_l_time = 0;
volatile uint16_t ENCR_max_h_time = 0;
...
ISR (INT0_vect)
{
	...
	uint16_t tmp = isEncoderLeft();
	if(ENCL_trig == TRIG_WAIT) {
		if(!tmp)
		{
			ENCL_min_l_time = 65535;
			ENCL_min_h_time = 65535;
			ENCL_max_l_time = 0;
			ENCL_max_h_time = 0;
			ENCL_trig=TRIG_ARMED;
		}
	}
	else if(ENCL_trig == TRIG_ARMED) {
		if(tmp) {
			ENCL_k = 0;
			ENCL_buffer_values[ENCL_k] = 0;
			ENCL_buffer_time[ENCL_k++] = 1;
			ENCL_trig=TRIGGERED;
			ENCL_time = 0;
		}				
	}
	else if(ENCL_trig == TRIGGERED) {
		if(ENCL_k < 51) {
			tmp = tmp ? 500 : 0;
			if(ENCL_time > 1000) {
				ENCL_buffer_values[ENCL_k] = tmp;
				ENCL_buffer_time[ENCL_k++] = ENCL_time;
			}
			else {
				ENCL_buffer_values[ENCL_k] = tmp;
				ENCL_buffer_time[ENCL_k++] = 1;
				ENCL_buffer_values[ENCL_k] = tmp ? 0 : 500;
				ENCL_buffer_time[ENCL_k++] = ENCL_time;
			}
			if(tmp)
			{
				if(ENCL_time > ENCL_max_h_time)
					ENCL_max_h_time = ENCL_time;
				if(ENCL_time < ENCL_min_h_time && ENCL_time > 0)
					ENCL_min_h_time = ENCL_time;
			}
			else
			{
				if(ENCL_time > ENCL_max_l_time)
					ENCL_max_l_time = ENCL_time;
				if(ENCL_time < ENCL_min_l_time && ENCL_time > 0)
					ENCL_min_l_time = ENCL_time;
			}
			ENCL_time = 0;
		}
		else 
			ENCL_trig = TRIG_PROCESS_DATA;
	}
}
...
ISR (INT1_vect)
{
	...
	uint16_t tmp = isEncoderRight();
	if(ENCR_trig == TRIG_WAIT) {
		if(!tmp)
		{
			ENCR_min_l_time = 65535;
			ENCR_min_h_time = 65535;
			ENCR_max_l_time = 0;
			ENCR_max_h_time = 0;
			ENCR_trig = TRIG_ARMED;
		}
	}
	else if(ENCR_trig == TRIG_ARMED) {
		if(tmp) {
			ENCR_k = 0;
			ENCR_buffer_values[ENCR_k] = 0;
			ENCR_buffer_time[ENCR_k++] = 1;
			ENCR_trig = TRIGGERED;
			ENCR_time = 0;
		}				
	}
	else if(ENCR_trig == TRIGGERED) {
		if(ENCR_k < 51) {
			tmp = tmp ? 500 : 0;
			if(ENCR_time > 700) {
				ENCR_buffer_values[ENCR_k] = tmp;
				ENCR_buffer_time[ENCR_k++] = ENCR_time;
			}
			else {
				ENCR_buffer_values[ENCR_k] = tmp;
				ENCR_buffer_time[ENCR_k++] = 1;
				ENCR_buffer_values[ENCR_k] = tmp ? 0 : 500;
				ENCR_buffer_time[ENCR_k++] = ENCR_time;
			}
			if(tmp)
			{
				if(ENCR_time > ENCR_max_h_time)
					ENCR_max_h_time = ENCR_time;
				if(ENCR_time < ENCR_min_h_time && ENCR_time > 0)
					ENCR_min_h_time = ENCR_time;
			}
			else
			{
				if(ENCR_time > ENCR_max_l_time)
					ENCR_max_l_time = ENCR_time;
				if(ENCR_time < ENCR_min_l_time && ENCR_time > 0)
					ENCR_min_l_time = ENCR_time;
			}
			ENCR_time = 0;
		}
		else 
			ENCR_trig = TRIG_PROCESS_DATA;
	}
}
